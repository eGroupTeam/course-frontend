...
const CustomerCreateEdit:React.FC<Props> = (props) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <DialogTitle>{props.customer.id===0?"新增顧客資料":"修改顧客資料"}</DialogTitle>
      <DialogContent>
        <TextField id="filled-basic" label="顧客姓名" variant="outlined" {...register("name",{ required: true, minLength: 2 })}/><br/>
        {errors.name && <span>姓名至少2個字<br/></span>}
        <TextField id="filled-basic" label="顧客地址" variant="outlined" {...register("address",{ required: true, minLength: 5 })}/><br/>
        {errors.address && <span>地址至少5個字<br/></span>}
        <TextField id="filled-basic" label="產品價格" variant="outlined" type="number" {...register("weight",{min:0, max:200})}/><br/>
        {errors.weight && <span>體重在0到200之間<br/></span>}
      </DialogContent>
      <DialogActions>
