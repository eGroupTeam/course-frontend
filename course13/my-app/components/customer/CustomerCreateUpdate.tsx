...
import {Customer, Product} from '@/interfaces/entities';
...
const CustomerCreateEdit:React.FC<Props> = (props) => {
  ...
  return (
    <div className={styles.container}>
      ...
      <DialogTitle>{props.customer.id===0?"新增顧客":"修改資料"}</DialogTitle>
      <DialogContent>
        <TextField id="filled-basic" label="顧客名稱" variant="outlined" {...register("name",{ required: true, minLength: 2 })}/><br/>
        {errors.name && <span>名字至少兩個字<br/></span>}
        <TextField id="filled-basic" label="顧客地址" variant="outlined" {...register("address",{ required: true, minLength: 2 })}/><br/>
        {errors.address && <span>地址至少兩個字<br/></span>}
        <TextField id="filled-basic" label="顧客體重" variant="outlined" type="number" {...register("weight",{min:1, max:1000})}/><br/>
        {errors.weight && <span>體重需介於1~1000<br/></span>}
      </DialogContent>
      <DialogActions>
